...
fn run_analyze(path: &str) {
    ...
    run_analyze_core(path, source_code);
}

fn run_analyze_core(path: &str, source_code: String) {
    let mut cache_mgr = cache::CacheManager::new();
    let file_hash = cache::CacheManager::compute_hash(&source_code);

    if let Some(cached_result) = cache_mgr.get_analysis(path, &file_hash) {
        println!("{}", serde_json::to_string_pretty(&cached_result).unwrap());
        return;
    }
...
fn run_fingerprint_core(path_str: &str, source: String) {
    ...
    if let Some(cached_result) = cache_mgr.get_fingerprint(path_str, &file_hash) {
        println!("{}", serde_json::to_string_pretty(&cached_result).unwrap());
        return;
    }
    ...
    match args[1].as_str() {
        "analyze" => {
            ...
            if args[2] == "-" && args.len() >= 4 {
                ...
                run_analyze_core(&args[3], buffer);
            } else {
                run_analyze(&args[2]);
            }
        }
