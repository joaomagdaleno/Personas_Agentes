...
5. Any change to the 186-test baseline MUST be noted in `state.md`.
...
| Agent | Conditions |
|---|---|
| Spec | (a) only adds new test files/cases, (b) does NOT modify existing test assertions, (c) all 186 tests pass, (d) coverage does not regress |
| Bolt | (a) all 186 tests pass, (b) coverage unchanged or improved, (c) benchmark shows >= 5% improvement, (d) no FFI/gRPC/SQLite/Idris files touched, (e) Sentinel has not flagged the file in last 7 days |
| Scribe | (a) only `.md` files or doc comments changed, (b) no code files touched, (c) `bun test` still passes with 186 tests |
| Refactor | (a) target coverage >= 80%, (b) all 186 tests pass, (c) coverage unchanged, (d) diff < 200 lines, (e) no public API signature changed, (f) no FFI/gRPC/SQLite/Idris files touched |
...
- Any PR changing the 186-test baseline in a non-additive way
...
6. An agent's action would change the 186-test baseline non-additively.
