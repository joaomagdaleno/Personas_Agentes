...
class NeuralPersona(BaseActivePersona):
    ...
    def __init__(self, project_root):
        ...
        self.mission = "AI-driven features and LLM logic."
        self.stack = "Python"

    def perform_audit(self) -> list:
        """Realiza auditoria técnica focada em IA."""
        issues = []
                    ...
                    try:
                        ...
                        if "openai" in content or "gemini" in content:
                            if "temperature" not in content:
                                issues.append({'file': rel_path, 'issue': 'Configuração de LLM sem definição de temperatura.', 'severity': 'low', 'context': 'Neural'})
                    except Exception:
                        continue
        return issues
