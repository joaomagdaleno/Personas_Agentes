...
class NebulaPersona(BaseActivePersona):
    ...
    def __init__(self, project_root):
        ...
        self.mission = "Cloud infrastructure and integration."
        self.stack = "Python"

    def perform_audit(self) -> list:
        """Realiza auditoria técnica focada em nuvem."""
        issues = []
                ...
                if file.endswith('.py'):
                    ...
                    try:
                        with open(os.path.join(root, file), 'r', encoding='utf-8', errors='ignore') as f:
                            content = f.read()
                        if "boto3" in content or "firebase" in content:
                            if "os.environ" not in content:
                                issues.append({'file': rel_path, 'issue': 'Configurações de nuvem sem uso de variáveis de ambiente.', 'severity': 'medium', 'context': 'Nebula'})
                    except Exception:
                        continue
        return issues
