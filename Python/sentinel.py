...
class SentinelPersona(BaseActivePersona):
    """Especialista em segurança Python."""
    def __init__(self, project_root):
        ...
        self.mission = "Secure the code and prevent leaks."
        self.stack = "Python"

    def perform_audit(self) -> list:
        """Realiza auditoria técnica focada em segurança."""
        issues = []
                ...
                if file.endswith('.py'):
                    ...
                    try:
                        with open(os.path.join(root, file), 'r', encoding='utf-8', errors='ignore') as f:
                            content = f.read()
                        if re.search(r'^[ \t]*password\s*=', content, re.M | re.I):
                            issues.append({'file': rel_path, 'issue': 'Possível segredo exposto.', 'severity': 'high', 'context': 'Sentinel'})
                    except Exception:
                        continue
        return issues
