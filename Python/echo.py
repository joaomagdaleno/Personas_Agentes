                ...
                if file.endswith('.py'):
                    ...
                    try:
                        with open(os.path.join(root, file), 'r', encoding='utf-8', errors='ignore') as f:
                            content = f.read()
                        if "input(" in content and "print(" not in content:
                            issues.append({'file': rel_path, 'issue': 'Input sem feedback visual detectado.', 'severity': 'low', 'context': 'Echo'})
                    except Exception:
                        continue
        return issues
