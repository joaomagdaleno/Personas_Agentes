...
export class ProbePersona extends BaseActivePersona {
    ...
    override async execute(context: ProjectContext): Promise<AuditFinding[]> {
        ...
        if (this.hub) {
            ...
            const reasoning = await this.hub.reason(`Analyze the Flutter resilience baseline given ${Errors.length} empty catch blocks in a graph of ${graph.nodes.length} nodes.`);
