...
export class DirectorPersona extends BaseActivePersona {
    ...
    public override async execute(context: ProjectContext): Promise<(AuditFinding | StrategicFinding)[]> {
        ...
        if (this.hub) {
            ...
            const reasoning = await this.hub.reason(`Synthesize an executive summary of the Flutter system's architecture and orchestration balance, given ${orchestratorNodes.length} orchestrator control nodes in ${this.stack}.`);
    ...
    override getAuditRules(): { extensions: string[]; rules: AuditRule[] } {
        return {
            extensions: ['.dart'],
            rules: [
                { regex: /void\s+main\s*\(/, issue: "Entry Point: Ponto de entrada detectado; verifique se o runApp e inicialização estão centralizados PhD.", severity: "low" },
                { regex: /MaterialApp|CupertinoApp/, issue: "App Root: Verifique se o widget raiz centraliza tema, rotas e providers PhD.", severity: "low" },
                { regex: /GetIt|Provider|Riverpod|BLoC/, issue: "State Management: Framework de estado detectado; garanta governança centralizada do estado global PhD.", severity: "medium" },
                { regex: /GoRouter|Navigator\.push/, issue: "Navigation: Verifique se a navegação segue um padrão declarativo e centralizado PhD.", severity: "low" },
                { regex: /Isolate\.spawn|compute\(/, issue: "Concurrency: Isolate detectado; verifique se o ciclo de vida está gerenciado corretamente PhD.", severity: "medium" },
    ...
    public override reasonAboutObjective(objective: string, _file: string, _content: string | Promise<string | null>): StrategicFinding | null {
        return {
            file: "orchestration",
            issue: `Direcionamento Estratégico Flutter para ${objective}: Garantindo alinhamento com a árvore de widgets e Clean Architecture PhD.`,
            severity: "STRATEGIC",
            context: this.name
        };
    }
