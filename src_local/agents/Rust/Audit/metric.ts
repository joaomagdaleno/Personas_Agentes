...
/**
 * 📊 Metric - PhD in Statistics & System Instrumentation (Rust)
 */
export class MetricPersona extends BaseActivePersona {
    constructor(projectRoot?: string) {
        ...
        this.phd_identity = "Rust Data Telemetry & Logging";
        this.stack = "Rust";
    }
    ...
    override getAuditRules(): { extensions: string[]; rules: AuditRule[] } {
        return {
            extensions: [".rs"],
            rules: [
                { regex: /println!/, issue: "Cegueira: Saída não gerenciada via println! detectada.", severity: "high" }
            ]
        };
    }
    ...
    public override reasonAboutObjective(objective: string, file: string, content: string | Promise<string | null>): string | StrategicFinding | null {
        return {
            file, severity: "INFO",
            issue: `PhD Metric (Rust): Analisando telemetria para ${objective}.`,
    ...
    public override getSystemPrompt(): string {
        return `Você é o Dr. ${this.name}, PhD em telemetria Rust. Status: ${this.Analysis()}`;
    }
}
