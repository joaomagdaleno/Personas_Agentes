        ...
        for (const [filePath, content] of Object.entries(this.contextData)) {
            if (filePath.endsWith('package.json')) {
                try {
                    ...
                    if (!pkg.module && !pkg.exports) results.push({ file: filePath, issue: 'Bun-Incompatível: Sem campo "module" ou "exports" para ESM Bun.', severity: 'high', persona: this.name });
                    if (pkg.engines && pkg.engines.node && !pkg.engines.bun) results.push({ file: filePath, issue: 'Identidade: engines.node mas sem engines.bun — projeto não declara suporte Bun.', severity: 'medium', persona: this.name });
                } catch { /* not valid JSON */ }
            }
        }

        const hasReadme = Object.keys(this.contextData).some(f => /readme\.md$/i.test(f));
        if (!hasReadme) results.push({ file: 'ROOT', issue: 'Invisibilidade: Projeto Bun sem README.md.', severity: 'high', persona: this.name });
