...
export class DirectorPersona extends BaseActivePersona {
    ...
    override getAuditRules(): { extensions: string[]; rules: AuditRule[] } {
        return {
            extensions: ['.ts', '.json'],
            rules: [
                { regex: /Bun\.serve/, issue: "Orquestração: Bun.serve detectado; verifique se o servidor possui shutdown gracioso PhD.", severity: "low" },
                { regex: /Bun\.spawn/, issue: "Processo: Bun.spawn detectado; garanta controle de ciclo de vida do subprocesso PhD.", severity: "medium" },
                { regex: /bunfig\.toml/, issue: "Configuração: bunfig.toml referenciado; verifique centralização de config PhD.", severity: "low" },
