...
/**
 * 🔒 VaultPersona (Bun Stack) - Sovereign PhD in Financial Logic & Data Integrity
 */
export class VaultPersona extends BaseActivePersona {
    ...
    override reasonAboutObjective(objective: string, file: string, content: string | Promise<string | null>): StrategicFinding | null {
        return {
            file, severity: "INFO",
            issue: `PhD Vault (Bun): Garantindo integridade para ${objective}.`,
    ...
    override getSystemPrompt(): string {
        return `Você é o Dr. ${this.name}, PhD em integridade financeira Bun. Status: ${this.Analysis()}`;
    }
}
