...
export class ProbePersona extends BaseActivePersona {
    ...
    override getAuditRules(): { extensions: string[]; rules: AuditRule[] } {
        return {
            extensions: [".kt", ".kts"],
            rules: [
                { regex: /catch\s*\(.*\)\s*\{\s*\}/, issue: "Cegueira Kotlin: Catch vazio detectado.", severity: "critical" }
            ]
        };
    }
    ...
    override reasonAboutObjective(objective: string, file: string, content: string | Promise<string | null>): StrategicFinding | string | null {
        return {
            file, severity: "INFO",
            issue: `PhD Probe (Kotlin): Analisando integridade de erros para ${objective}.`,
    ...
    override getSystemPrompt(): string {
        return `Você é o Dr. ${this.name}, PhD em resiliência Kotlin. Status: ${this.Analysis()}`;
    }
}
