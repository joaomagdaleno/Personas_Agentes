...
/**
 * 🏎️ Dr. Bolt — PhD in Kotlin Computational Efficiency
 */
export class BoltPersona extends BaseActivePersona {
    ...
    getAuditRules(): { extensions: string[]; rules: AuditRule[] } {
        return {
            extensions: ['.kt'],
            rules: [
                { regex: /while\s*\(true\)\s*\{/, issue: 'Gargalo: Loop infinito detectado em Kotlin.', severity: 'critical' }
            ]
        };
    }
    ...
    override reasonAboutObjective(objective: string, file: string, content: string | Promise<string | null>): StrategicFinding | string | null {
        return {
            file, severity: "INFO",
            issue: `PhD Bolt (Kotlin): Analisando eficiência para ${objective}.`,
    ...
    override getSystemPrompt(): string {
        return `Você é o Dr. ${this.name}, mestre em performance Kotlin. Status: ${this.Analysis()}`;
    }
}
