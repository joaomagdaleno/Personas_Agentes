...
export class DiagnosticPipeline {
    ...
    async execute(options: { skipTests?: boolean, autoHeal?: boolean } = {}): Promise<Path> {
        ...
        try {
            return await this.runAtomicPipeline(options.skipTests || false, options.autoHeal || false);
        } finally {
            DiagnosticPipeline._isRunning = false;
        }
    }

    private async runAtomicPipeline(skip: boolean, autoHeal: boolean): Promise<Path> {
        ...
        const result = await DiagnosticFinalizer.finalize(this, ctx, health, this.deduplicator.deduplicate(findings));
