export interface PsaApprovalRequest {
    callId: string;
    sessionId: string;
    toolName: string;
    args: any;
 ...
 */
export class PsaApprovalManager {
    private pending: Map<string, PsaApprovalRequest> = new Map();
    ...
    public requestApproval(params: { callId: string; sessionId: string; toolName: string; args: any; autoApproveIfTest?: boolean }): Promise<boolean> {
        if (params.autoApproveIfTest || process.env.PSA_AUTO_APPROVE === "true" || process.env.DSH_AUTO_APPROVE === "true") {
            return Promise.resolve(true);
        }

        return new Promise<boolean>((resolve) => {
            const req: PsaApprovalRequest = {
    ...
    public getPending(): PsaApprovalRequest[] {
        return Array.from(this.pending.values());
    }

    public get(callId: string): PsaApprovalRequest | undefined {
        return this.pending.get(callId);
    }
}

// Compatibilidade
export { PsaApprovalManager as DshApprovalManager };
export type ApprovalRequest = PsaApprovalRequest;
