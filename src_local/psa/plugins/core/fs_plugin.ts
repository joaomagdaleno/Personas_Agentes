...
export class FSPlugin implements PsaPlugin {
    public name = "psa-plugin-fs";
    ...
    public apply(ctx: PsaContext): void {
        const resolveSafePath = (targetPath: string): string => {
            ...
            if (!resolved.startsWith(root)) {
                throw new Error(`[PSA FS Security] Acesso negado: o caminho '${targetPath}' viola os limites do workspace (${root}).`);
            }
            return resolved;
        };
